...
class PredictLoss(nn.Module):
    def __init__(self, use_target_weight):
        ...
        self.heatmap_loss = JointsMSELoss(use_target_weight)

    def forward(self, output, target, target_weight):
        return self.heatmap_loss(output, target, target_weight)
...
class GenerativeLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        for idx in range(num_joints):
            ...
            # print(real_or_fake_pred)
            # print(real_or_fake_gt)
            loss += self.criterion(real_or_fake_pred, real_or_fake_gt)

        return loss / num_joints


class LSGenerativeLoss(nn.Module):
    def __init__(self):
        super(LSGenerativeLoss, self).__init__()
        self.criterion_D = nn.MSELoss()

    def forward(self, output, target):
        loss = self.criterion_D(output, target)
        return loss
